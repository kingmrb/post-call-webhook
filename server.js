...
// ============================================
// ROTI'S INDIAN RESTAURANT SERVER - VERSION 1.5.2
// Last Updated: July 2025
// Features: Using /get-total items with AI-parsed spice levels/notes from final "Your final order is", default mild spice level for biryanis/entrees, improved appetizer parsing with spice levels, robust contact info extraction, fixed async issue in extractItemsFromTranscript
// ============================================
const app = express();
...
Extract EVERY SINGLE ITEM with correct quantities
"two chicken biryanis both with mild" = quantity: 2, NOT two separate entries
Pay attention to quantity words: one=1, two=2, three=3, four=4, five=5
"both" means the quantity applies to both/all items (don't create duplicates)
For biryanis/entrees, extract spice levels; if none specified, use "mild"
For other items (e.g., appetizers), include spice levels if specified, but do not require them
Extract any notes or modifications (e.g., "no onions", "extra sauce") in a "notes" field
"sixty-five" or "65" in "chicken sixty-five biryani" is part of the dish name
Handle phrases like "with mild" or "spicy" correctly for all items
...
.replace(/\bwith\s+(very mild|mild|medium|spicy|hot|extra spicy|very hot)\b(?!\s*(&|and)\ssour)/gi, '') // Remove "with [spice level]"
.replace(/\b(very mild|mild|medium|spicy|hot|extra spicy|very hot)\b(?!\s(&|and)\ssour)/gi, '') // Remove standalone spice levels
...
let confirmationIndex = -1;
const patterns = [
/your final order is[:\s](.+?)(?:.\sis that correct|?\sis that correct|is that correct)/i,
/got it[!.]?\syour final order is[:\s](.+?)(?:.\sis that correct|?\sis that correct|is that correct)/i,
/here's your order[:\s](.+?)(?:.\sis that correct|?\sis that correct|is that correct)/i,
/to confirm[,:]\s*(.+?)(?:.\sis that correct|?\sis that correct|is that correct)/i
];
...
res.json({
status: 'ok',
service: 'Roti's Indian Restaurant Price Calculator v1.5.2',
...
app.listen(port, () => {
console.log('============================================');
console.log('✅ Roti's Indian Restaurant Server v1.5.2 - Started Successfully');
console.log(📍 Listening on port ${port});
console.log('🔄 Features: Using /get-total items with AI-parsed spice levels/notes from final "Your final order is", default mild spice level for biryanis/entrees, improved appetizer parsing with spice levels, robust contact info extraction, fixed async issue in extractItemsFromTranscript');
