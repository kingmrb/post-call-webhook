...
async function extractOrderFromSummary(summary, fallbackTranscript) {
  // FIRST: Check if we have a recent complete order from get-total (within 10 minutes)
...
process.on('SIGINT', () => {
  ...
  process.exit(0);
}); + latestCompleteOrder?.subtotal || '0.00',
      tax: '
}
...
process.on('SIGINT', () => {
  console.log('SIGINT received, shutting down gracefully...');
  process.exit(0);
}); + latestCompleteOrder?.total || '0.00'
    };
  }
