...
// Parse order text directly from "Your final order is" statement
function parseOrderTextDirectly(orderText) {
  const items = [];
  
  console.log('📝 Parsing order text directly:', orderText);
  
  // Split the order into segments by commas and "and"
  let segments = [];
  
  // Split by commas first, then handle "and" more carefully
  const commaParts = orderText.split(',');
  
  for (let i = 0; i < commaParts.length; i++) {
    let part = commaParts[i].trim();
    
    // Handle the last part which might have "and" before the final item
    if (i === commaParts.length - 1 && part.includes(' and ')) {
      // Split the last part by "and" to get the final items
  ...
  for (const segment of segments) {
    ...
    let quantity = 1;
    let itemText = segment.trim();
    
    // Extract quantity from the beginning (one, two, three, etc. or numbers)
    const qtyMatch = itemText.match(/^(one|two|three|four|five|six|seven|eight|nine|ten|\d+)\s+(.+)/i);
    if (qtyMatch) {
      quantity = QUANTITY_WORDS[qtyMatch[1].toLowerCase()] || parseInt(qtyMatch[1]) || 1;
      itemText = qtyMatch[2].trim();
      console.log(`  Quantity: ${quantity}`);
    }
    
    // Clean the item text (remove extra words)
    const cleanedText = itemText
      .replace(/\b(orders?\s+of|pieces?\s+of)\s*/gi, '')
      .replace(/\b(the|of|a|an)\b/gi, ' ')
      .replace(/\s+/g, ' ')
      .trim();
    ...
    if (menuItem && MENU_ITEMS[menuItem]) {
      ...
      addItemToOrder(items, menuItem, quantity, price, []);
    } else {
      console.log(`  ❌ Could not match: "${cleanedText}"`);
    }
  }
  
  console.log(`✅ Parsed ${items.length} items from order text`);
  return items;
}
  const items = [];
...
async function extractOrderFromSummary(summary, fallbackTranscript) {
  // FIRST: Check if we have recent stored order from get-total (within 10 minutes)
  const tenMinutesAgo = Date.now() - (10 * 60 * 1000);
  
  if (latestCompleteOrder && latestOrderTimestamp && latestOrderTimestamp > tenMinutesAgo) {
    console.log('✅ Using complete order from get-total endpoint');
    ...
    // Format items for Toast integration
    const formattedItems = latestCompleteOrder.items.map(item => ({
      name: item.name,
    ...
    // Store the totals before clearing
    const orderSubtotal = latestCompleteOrder.subtotal;
    const orderTax = latestCompleteOrder.tax;
    const orderTotal = latestCompleteOrder.total;
    
    // Clear the stored order after using it
    latestCompleteOrder = null;
...
process.on('SIGINT', () => {
  ...
  // SECOND: Look for "Your final order is" in transcript (most reliable for exact menu items)
  if (fallbackTranscript && fallbackTranscript.length > 0) {
    console.log('🎯 Looking for "Your final order is" in transcript');
    
    const fullConversation = fallbackTranscript.map(t => t.message).join(' ').toLowerCase();
    
    // Find ALL occurrences of "Your final order is" and use the LAST one
    const regex = /your final order is[:\s]*(.+?)(?:\.\s*is that correct|\?\s*is that correct|is that correct)/gi;
    const matches = [...fullConversation.matchAll(regex)];
    
    if (matches.length > 0) {
      // Use the LAST occurrence (most recent/final order confirmation)
      const lastMatch = matches[matches.length - 1];
      let orderText = lastMatch[1].trim();
      ...
      console.log(`✅ Found ${matches.length} "Your final order is" occurrences, using the LAST one`);
      console.log('📍 Final order text:', orderText);
      
      // Parse this order text directly using regex parsing
      const transcriptItems = parseOrderTextDirectly(orderText);
      
      if (transcriptItems && transcriptItems.length > 0) {
        console.log('✅ Successfully extracted items from final order confirmation');
      ...
      console.log('⚠️ No "Your final order is" found in transcript');
    }
  }

  // THIRD: Fallback to general transcript parsing
  if (fallbackTranscript && fallbackTranscript.length > 0) {
    console.log('🔄 Falling back to general transcript parsing');
    
    const transcriptItems = extractItemsFromTranscript(fallbackTranscript);
    
    if (transcriptItems && transcriptItems.length > 0) {
      console.log('✅ Successfully extracted items using general transcript parsing');
  ...
  // Final fallback - no order detected
  return null;
}
